...
|---|---|
| 轮速 | 左轮速度、右轮速度、整车速度 |
| 磨盘 | 磨盘设置转速、磨盘开关、磨盘升降状态 |
| 工作模式 | 自动/手动等模式状态 |
| 照明 | 灯光开关状态 |
| 底盘 | 底盘使能状态 |
| 位置 | 当前 `x/y/heading`、定位协方差、定位质量（0～100） |
| 避障 | `collision_imminent`，来源 `/rpp/collision_imminent`，`true` 表示导航检测到即将碰撞 |
| 雷达 | `radar_system_status_available`、`radar_system_status`，来源 `/slamware_ros_sdk_server_node/system_status` |
...
`RadarSystemStatusResponse` 中，`available` 表示调度程序是否已经收到雷达状态，
`status` 保留雷达原始状态字符串，`timestamp_ns` 保留雷达消息时间戳。尚未收到状态时
返回 `RESULT_FAILED` 和 `radar_system_status_unavailable`。
