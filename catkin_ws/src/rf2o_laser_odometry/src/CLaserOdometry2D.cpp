...
bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::LaserScan& scan)
{
  ...
  // Copy the scan while rejecting invalid ranges.  LaserScan commonly uses
  // +/-inf for no-return beams; feeding those values to the image pyramid
  // creates NaNs (for example inf - inf), which makes the RF2O solution stay
  // at its initial pose.
  for (unsigned int i = 0; i < width; ++i)
  {
    const float range_value = scan.ranges[i];
    range_wf(i) = std::isfinite(range_value) &&
                  range_value >= scan.range_min &&
                  range_value <= scan.range_max
                    ? range_value
                    : 0.f;
  }
