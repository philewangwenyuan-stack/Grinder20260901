...
//获取栅格地图
bool GetPgmCallback(super_lio::SetMap::Request& req,
                    super_lio::SetMap::Response& res)
{
    ROS_INFO("SetMap: Request received.");
    pcd_file = req.map_path + "/" + req.map_name + pcd_format;
    // 下载pcd文件
    if (pcl::io::loadPCDFile<pcl::PointXYZ>(pcd_file, *pcd_cloud) == -1) {
      PCL_ERROR("Couldn't read file: %s \n", pcd_file.c_str());
      return (-1);
    }
    ROS_INFO("[pcd2pgm] 初始点云数据点数：%d", pcd_cloud->points.size());
    //对数据进行直通滤波
...
int main(int argc, char **argv) {
  ...
  ros::ServiceServer map_server = nh.advertiseService("/pcd2pgm/get_pgm", GetPgmCallback);
...
void PassThroughFilter(const double &thre_low, const double &thre_high,
  ...
  ROS_INFO("[pcd2pgm] 直通滤波后点云数据点数：%d", cloud_after_PassThrough->points.size());
}
...
void RadiusOutlierFilter(const pcl::PointCloud<pcl::PointXYZ>::Ptr &pcd_cloud0,
  ...
  //                                     *cloud_after_Radius);
  ROS_INFO("[pcd2pgm] 半径滤波后点云数据点数：%d", cloud_after_Radius->points.size());
}
