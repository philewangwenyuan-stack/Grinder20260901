...
{    
    ...
    public:
        ...
        // Software state machine for the relocalization status topic. The device firmware
        // latches the last relocalization result, so the node keeps its own logical state,
        // and the status worker renders it with a short hold on terminal states before
        // falling back to None.
        enum RelocalizationLogicState
        {
            RELOC_STATE_NONE = 0,
            RELOC_STATE_RUNNING = 1,
            RELOC_STATE_SUCCEED = 2,
            RELOC_STATE_FAILED = 3,
            RELOC_STATE_CANCELED = 4,
        };
        int relocalizationLogicState() const { return relocState_.load(); }
        void setRelocalizationLogicState(int s) { relocState_.store(s); }
    ...
    private:
        ...
        //////////////////////////////////////////////////////////////////////////
        void checkRelocalizationStatus(bool &saw_in_progress);
    ...
    private:
        ...
        std::atomic<int> relocState_;   // RelocalizationLogicState, see public accessors
    };
