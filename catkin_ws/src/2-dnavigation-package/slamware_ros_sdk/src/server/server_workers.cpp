...
namespace slamware_ros_sdk
{
    ...
    namespace
    {
        ...
        // Aurora stores a column-major SE(3) covariance ordered as
        // [rotation x/y/z, translation x/y/z]. ROS stores a row-major
        // covariance ordered as [x, y, z, roll, pitch, yaw].
        bool convertAuroraPoseCovarianceToRos(
            const rp::standalone::aurora::PoseCovariance &auroraCovariance,
            boost::array<double, 36> &rosCovariance)
        {
            static const int rosToAurora[6] = {3, 4, 5, 0, 1, 2};
            for (int rosRow = 0; rosRow < 6; ++rosRow)
            {
                for (int rosCol = 0; rosCol < 6; ++rosCol)
                {
                    const double value = auroraCovariance(
                        rosToAurora[rosRow], rosToAurora[rosCol]);
                    if (!std::isfinite(value))
                    {
                        return false;
                    }
                    rosCovariance[rosRow * 6 + rosCol] = value;
                }

                if (rosCovariance[rosRow * 6 + rosRow] < 0.0)
                {
                    return false;
                }
            }
        ...
        void fillUnavailablePoseCovariance(double variance,
                                           boost::array<double, 36> &covariance)
        {
            covariance.assign(0.0);
            const double safeVariance =
                std::isfinite(variance) && variance > 0.0 ? variance : 1.0e6;
            for (int i = 0; i < 6; ++i)
            {
                covariance[i * 6 + i] = safeVariance;
            }
        }

        void fillDerivedTwistCovariance(const ServerParams &srvParams,
                                        boost::array<double, 36> &covariance)
        {
            covariance.assign(0.0);
            const double unavailable =
                std::isfinite(srvParams.odom_covariance_unavailable_variance) &&
                        srvParams.odom_covariance_unavailable_variance > 0.0
                    ? srvParams.odom_covariance_unavailable_variance
                    : 1.0e6;
            const double linear =
                std::isfinite(srvParams.odom_twist_linear_variance) &&
                        srvParams.odom_twist_linear_variance > 0.0
                    ? srvParams.odom_twist_linear_variance
                    : 0.0025;
            const double angular =
                std::isfinite(srvParams.odom_twist_angular_variance) &&
                        srvParams.odom_twist_angular_variance > 0.0
                    ? srvParams.odom_twist_angular_variance
                    : 0.01;

            covariance[0] = linear;       // vx
            covariance[7] = linear;       // vy
            covariance[14] = unavailable; // vz is not calculated
            covariance[21] = unavailable; // roll rate is not calculated
            covariance[28] = unavailable; // pitch rate is not calculated
            covariance[35] = angular;      // yaw rate
        }
    ...
    void ServerRobotPoseWorker::doPerform()
    {
        ...
        wkDat->robotPoseSdkTimestampNs.store(pose_timestamp);
...
//         tfBrdcst.sendTransform(t);
    ...
    void ServerOdometryWorker::doPerform()
    {
        ...
        bool hasValidPoseCovariance = false;
        auto auroraSDK = rosSdkServer()->safeGetAuroraSdk();
        if (srvParams.use_aurora_pose_covariance && auroraSDK)
        {
            rp::standalone::aurora::PoseCovariance auroraCovariance;
            uint64_t covarianceTimestampNs = 0;
            if (auroraSDK->dataProvider.getRecentPoseCovariance(
                    auroraCovariance, &covarianceTimestampNs))
            {
                const uint64_t poseTimestampNs =
                    wkDat->robotPoseSdkTimestampNs.load();
                const uint64_t timestampDeltaNs =
                    poseTimestampNs > covarianceTimestampNs
                        ? poseTimestampNs - covarianceTimestampNs
                        : covarianceTimestampNs - poseTimestampNs;
                const double covarianceAge =
                    static_cast<double>(timestampDeltaNs) * 1.0e-9;
                const double maxAge =
                    std::max(0.0, srvParams.aurora_covariance_max_age);

                if (poseTimestampNs != 0 && covarianceAge <= maxAge)
                {
                    hasValidPoseCovariance = convertAuroraPoseCovarianceToRos(
                        auroraCovariance, odom.pose.covariance);
                }
                else
                {
                    ROS_WARN_THROTTLE(5.0,
                        "Aurora pose covariance is stale (age %.3f s, limit %.3f s)",
                        covarianceAge, maxAge);
                }
            }
            else
            {
                ROS_WARN_THROTTLE(5.0,
                    "Aurora pose covariance is unavailable; firmware 2.1.1+ and active tracking are required");
            }
        }

        if (!hasValidPoseCovariance)
        {
            fillUnavailablePoseCovariance(
                srvParams.odom_covariance_unavailable_variance,
                odom.pose.covariance);
        }
        fillDerivedTwistCovariance(srvParams, odom.twist.covariance);
    ...
    void ServerSystemStatusWorker::doPerform()
    {
        ...
        // ---- Relocalization status: software state machine (per spec) ----
        // The device firmware latches the last relocalization result until the next attempt,
        // so the node keeps its own logical state machine (driven by request / cancel /
        // algorithm result) instead of mirroring the device latch directly:
        //   None --start command--> Running --algorithm result--> Succeed / Failed
        //   Running --manual cancel--> Canceled
        //   Idle (before any request) keeps publishing RelocalizationNone.
        //   Terminal states (Succeed / Failed / Canceled) are held for a short window, then the
        //   node STOPS publishing the topic until the next relocalization request (Running).
        {
            constexpr std::chrono::seconds kRelocTerminalHold(3);
            slamware_ros_sdk::RelocalizationStatus reloc_resp;
            reloc_resp.timestamp_ns = ros::Time::now().toNSec();

            const int ls = rosSdkServer()->relocalizationLogicState();
            std::string state_str;
            switch (ls)
            {
            case SlamwareRosSdkServer::RELOC_STATE_RUNNING:
                state_str = "RelocalizationRunning";
                break;
            case SlamwareRosSdkServer::RELOC_STATE_SUCCEED:
                state_str = "RelocalizationSucceed";
                break;
            case SlamwareRosSdkServer::RELOC_STATE_FAILED:
                state_str = "RelocalizationFailed";
                break;
            case SlamwareRosSdkServer::RELOC_STATE_CANCELED:
                state_str = "RelocalizationCanceled";
                break;
            default:
                state_str = "RelocalizationNone";
                break;
            }

            if (ls == SlamwareRosSdkServer::RELOC_STATE_NONE || ls == SlamwareRosSdkServer::RELOC_STATE_RUNNING)
            {
                // Idle / running: publish as-is and clear any held terminal state.
                relocHeldTerminal_.clear();
                reloc_resp.status = state_str;
                pubRelocalizaitonStatus_.publish(reloc_resp);
            }
            else
            {
                // Terminal state: hold for kRelocTerminalHold, then STOP publishing.
                // The topic stays silent until the next request moves the state to Running.
                const auto now = std::chrono::steady_clock::now();
                if (relocHeldTerminal_ != state_str)
                {
                    relocHeldTerminal_ = state_str;
                    relocHoldStart_ = now;
                }
                if (now - relocHoldStart_ < kRelocTerminalHold)
                {
                    reloc_resp.status = state_str;
                    pubRelocalizaitonStatus_.publish(reloc_resp);
                }
                // else: hold expired -> do not publish (silent until next request).
            }
        }
    }
