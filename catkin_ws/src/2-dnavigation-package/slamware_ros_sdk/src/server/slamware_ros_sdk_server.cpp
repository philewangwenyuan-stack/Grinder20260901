...
namespace slamware_ros_sdk
{
        ...
        const RelocalizationCancelRequest::ConstPtr &msg)
    {
        ...
        relocState_.store(RELOC_STATE_CANCELED);
    }
        ...
        slamware_ros_sdk::RelocalizationRequest::Response &resp)
    {
        ...
        relocState_.store(RELOC_STATE_RUNNING);

        relocalization_future_ = std::async(std::launch::async, [this]()
                                            {
            // The device latches the last relocalization result. Right after a new request,
            // getLastRelocalizationStatus() may still return the previous (stale) result until
            // the device processes the request, so only accept a terminal result (SUCCEED/FAILED)
            // after the device has entered IN_PROGRESS.
            bool saw_in_progress = false;
            auto request_start = std::chrono::steady_clock::now();
            while (true)
            {
                checkRelocalizationStatus(saw_in_progress);
                if (cancel_requested_.load() || !relocalization_active_.load())
                {
                    return;
                }
                // Guard: if the device never starts the relocalization, abort to avoid blocking
                // future relocalization requests forever.
                if (!saw_in_progress &&
                    std::chrono::steady_clock::now() - request_start > std::chrono::seconds(5))
                {
                    ROS_ERROR("Relocalization did not start within 5s, aborting");
                    relocalization_active_.store(false);
                    relocState_.store(RELOC_STATE_NONE);
                    return;
                }
    ...
    void SlamwareRosSdkServer::checkRelocalizationStatus(bool &saw_in_progress)
    {
        auto aurora = safeGetAuroraSdk();
        if (!aurora)
        {
        ...
        if (last_status == SLAMTEC_AURORA_SDK_DEVICE_RELOCALIZATION_STATUS_IN_PROGRESS)
        {
            // The new relocalization has started; terminal results are trustworthy from now on.
            saw_in_progress = true;
        }
        else if (saw_in_progress && last_status == SLAMTEC_AURORA_SDK_DEVICE_RELOCALIZATION_STATUS_SUCCEED)
        {
            ROS_INFO("Relocalization succeeded");
            relocalization_active_.store(false);
            relocState_.store(RELOC_STATE_SUCCEED);
        }
        else if (saw_in_progress && last_status == SLAMTEC_AURORA_SDK_DEVICE_RELOCALIZATION_STATUS_FAILED)
        {
            ROS_ERROR("Relocalization failed");
            relocalization_active_.store(false);
            relocState_.store(RELOC_STATE_FAILED);
        }
        // NONE, or a terminal result read before IN_PROGRESS (stale latch from a previous
        // relocalization): keep waiting.
    }
